import { Injectable, Inject, Optional, ExecutionContext, CanActivate, NotFoundException } from '@nestjs/common';
...
@Injectable() export class OrchestratorGuard implements CanActivate {
	...
	public async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		const guardKeys = this.reflector.getAllAndOverride<string[]>(USE_GUARDS_KEY, [
		...
		const keysToExecute = guardKeys?.length ? guardKeys : (

			this.defaultGuardKey ? [ this.defaultGuardKey ] : []
